...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        // SQLite (test suite) — portable Schema builder. cpt_codes uses
        // json() instead of jsonb(); SQLite doesn't distinguish and
        // Eloquent's array cast handles serialization either way.
        Schema::table('encounters', function (Blueprint $table) {
            if (!Schema::hasColumn('encounters', 'duration_minutes_actual')) $table->integer('duration_minutes_actual')->nullable();
            if (!Schema::hasColumn('encounters', 'time_spent_documenting')) $table->integer('time_spent_documenting')->nullable();
            if (!Schema::hasColumn('encounters', 'total_time_minutes')) $table->integer('total_time_minutes')->nullable();
            if (!Schema::hasColumn('encounters', 'cpt_codes')) $table->json('cpt_codes')->nullable();
            if (!Schema::hasColumn('encounters', 'units_billed')) $table->integer('units_billed')->nullable();
            if (!Schema::hasColumn('encounters', 'bill_status')) $table->string('bill_status', 20)->default('not_billed');
            if (!Schema::hasColumn('encounters', 'cosigner_user_id')) {
                $table->foreignUuid('cosigner_user_id')->nullable()->constrained('users')->nullOnDelete();
            }
            if (!Schema::hasColumn('encounters', 'cosigned_at')) $table->timestamp('cosigned_at')->nullable();
        });
